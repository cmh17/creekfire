...
outDir <- file.path(wd, "data", "openet", fsep="/")
...
names(selected_boxes) <- 1:length(selected_boxes)
names(box_geometries) <- 1:length(selected_boxes)

# plot each box to check coverage
# plot(boundary_vect)
# for (i in 1:length(box_geometries)) {
#   lines(x=box_geometries[[i]][c(1,3,5,7,1)],y=box_geometries[[i]][c(2,4,6,8,2)],type="l")
# }
###############################################################################
...
for (i in 1:length(box_geometries)){
  ...
  for (j in 1:length(date_ranges)) {
    ...
    for (k in 1:length(response_urls)) {
      ...
      zip_dir <- file.path(wd, "data/openet/zips", paste0("box",names(selected_boxes)[[i]]), fsep="/")
      tif_dir <- file.path(wd, "data/openet/tiffs", paste0("box",names(selected_boxes)[[i]]), fsep="/")
...
for (i in 1:length(selected_boxes)){
  ...
  dir_str <- paste0("data/openet/tiffs/",paste0("box",selected_boxes[i]))
