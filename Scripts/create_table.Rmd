...
# plot(slope_resampled)
# 
# terra::writeRaster(postfire_et,paste0(wd,"/Data/SRTM_data/SRTM_mosaic_resampled.tif"),
#                    overwrite=TRUE)
# terra::writeRaster(temperature_resampled,paste0(wd,"/Data/NLDAS2_DAILY_tasmean_2020-03-04_2020-09-04_resampled.tif"),
#                    overwrite=TRUE)
# terra::writeRaster(temperature,paste0(wd,"/Data/NLDAS2_DAILY_tasmean_2020-03-04_2020-09-04_resampled.tif"),
#                    overwrite=TRUE)
# terra::writeRaster(precipitation_resampled,paste0(wd,"/Data/TERRACLIMATE_pr_2020-03-01_2020-08-31_resampled.tif"),
#                    overwrite=TRUE)
# terra::writeRaster(soil_moisture_resampled,paste0(wd,"/Data/TERRACLIMATE_soil_2020-03-01_2020-08-31_resampled.tif"),
#                    overwrite=TRUE)
# terra::writeRaster(postfire_et_resampled,paste0(wd,"/Data/openET_data/tiffs/postfire_mean_ET_resampled.tif"),
#                    overwrite=TRUE)
# terra::writeRaster(postfire_et_resampled,paste0(wd,"/Data/openET_data/tiffs/rasterized_creekfire_geojson.tif"),
#                    overwrite=TRUE)
# terra::writeRaster(slope_resampled,paste0(wd,"/Data/SRTM_data/SRTM_slope_resampled.tif"),
#                    overwrite=TRUE)
# terra::writeRaster(aspect_resampled,paste0(wd,"/Data/SRTM_data/SRTM_aspect_resampled.tif"),
#                    overwrite=TRUE)
# 
# # diff mean NDVI
# diff_mean_ndvi <- terra::rast(paste0(wd,"/Data/HLS_data/diff_mean_ndvi.tif"))
# 
# # diff mean NBR
# diff_mean_nbr <- terra::rast(paste0(wd,"/Data/HLS_data/diff_mean_nbr.tif"))
# 
# # NDVI and NBR should already be resampled... I hope
```
...
# Check if each point is within the boundary
# boundary_check <- function(lat, lon, boundary_vec) {
#   # Create a SpatVector from the coordinates
#   points <- vect(data.frame(x = lon, y = lat), geom = c("x", "y"), crs = crs(boundary_vec))
#   
#   # Perform intersection to check if points fall within the boundary
#   in_boundary <- !is.na(terra::intersect(points, boundary_vec))
#   
#   return(in_boundary)
# }
...
data_outside_buffer <- data_table[data_table$within_boundary==0]
head(data_outside_buffer,20)

ggplot(data_table[1:1000000,], aes(lon,lat)) +
  geom_tile(aes(fill = dNDVI))
