...
boundary_vect <- terra::vect(paste0(wd,"/data/boundaries/creek_fire_buffer.geojson"))

# get spatial extent of boundary
roi <- terra::ext(boundary_vect) # ext gets a spatial extent

# Load openet data to use as a reference for reprojection
prefire_et <- terra::rast(paste0(wd,"/data/openet/tiffs/prefire_mean_et.tif"))
